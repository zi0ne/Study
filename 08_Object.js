describe('Object에 대해서 학습합니다.', function () {
  ...
  */
  it('Object의 기본을 확인합니다.', function () {
    const emptyObj = {};
    expect(typeof emptyObj === 'object').to.equal(true); // typeof로 객체를 가려낼 수 있다.
    expect(emptyObj.length).to.equal(undefined); // 객체의 키의 길이는 알 수 있지만, 객체의 길이는 정의되어있지 않다.
  ...
  it('Object의 속성(property)를 다루는 방법을 확인합니다.', function () {
    ...
    expect(megalomaniac['mastermind']).to.equal('Neo'); // 40번에서 neo로 바꿈

    expect('secretary' in megalomaniac).to.equal(false); // secretary는 없음 

    megalomaniac.secretary = 'Agent Smith';// {secretary:'Agent Smith'}추가
    expect('secretary' in megalomaniac).to.equal(true);

    delete megalomaniac.henchman; // 지우기
    expect('henchman' in megalomaniac).to.equal(false);
  });

  it("'this'는 method를 호출하는 시점에 결정됩니다.", function () {
    const currentYear = new Date().getFullYear();// getFullYear 현지의 년도를 불러오는 메서드
    const megalomaniac = {
    ...
    expect(megalomaniac.calculateAge(currentYear)).to.equal(53);//2023-1970

    megalomaniac.birthYear = 2000;
    expect(megalomaniac.calculateAge(currentYear)).to.equal(23);//2023 - 2000

    megalomaniac.changeBirthYear(2010);
    expect(megalomaniac.calculateAge(currentYear)).to.equal(13);//2023-2010
  ...
  it('객체의 method를 정의하는 방법을 확인합니다.', function () {
    const megalomaniac = {
      ...
      battleCry(numOfBrains) {
        return `They are ${this.henchman} and the` + ` ${this.mastermind}`.repeat(numOfBrains);// 리핏 = 반복
      },
    };
  ...
  it('Object를 함수의 전달인자로 전달할 경우, reference가 전달됩니다.', function () {
    ...
    expect(obj.henchwoman).to.equal('Adam West'); // 함수 안에서 값이 변함

    const assignedObj = obj;
    assignedObj['relations'] = [1, 2, 3];  // 주소 공유 후, 바꿈
    expect(obj['relations']).to.deep.equal([1,2,3]);

    const copiedObj = Object.assign({}, obj); // 새로운 객체에 obj를 복사하는 것, obj에는 변화가 없음
    copiedObj.mastermind = 'James Wood';
    expect(obj.mastermind).to.equal('Joker');

    obj.henchwoman = 'Harley';
    expect(copiedObj.henchwoman).to.equal('Adam West'); // 함수는 이미 그 전의 obj를 복사해 갔음
